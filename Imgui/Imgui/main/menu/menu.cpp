                ...
                switch (active_tab) {
                    ...
                    case 4: {
                        ...
                        if (show_style_editor) {
                            ImGui::Begin(u8"样式编辑器", &show_style_editor, window_flags);
                        ...
                        ImGui::Separator();
                        ImGui::Checkbox(u8"锁定菜单布局", &menu::Settings::lock_window_size);
...
namespace menu {
    /*void DrawFullscreenGrid()
    {
        ...
        const float grid_spacing = 50.0f; // 网格间距
        const ImU32 grid_color = IM_COL32(120, 120, 120, 120);

        // 3. 绘制垂直线
        for (float x = 0; x < display_size.x; x += grid_spacing) {
            bg_draw_list->AddLine(
                ImVec2(x, 0),                 // 起点 (屏幕顶部)
                ImVec2(x, display_size.y),    // 终点 (屏幕底部)
                grid_color
            );
        }
        ...
        for (float y = 0; y < display_size.y; y += grid_spacing) {
            bg_draw_list->AddLine(
                ImVec2(0, y),                 // 起点 (屏幕左侧)
                ImVec2(display_size.x, y),    // 终点 (屏幕右侧)
                grid_color
            );
        }
    }
    ...
    {
        // 如果鼠标在窗口外，ImGui可能不知道它的位置，所以我们加个判断
        ...
        const ImU32 crosshair_color = IM_COL32(255, 0, 0, 150); // 半透明红色

        // 3. 绘制水平线 (从屏幕左边到右边，Y坐标为鼠标Y)
        fg_draw_list->AddLine(
            ImVec2(0, mouse_pos.y),
            ImVec2(display_size.x, mouse_pos.y),
            crosshair_color,
            1.0f // 线条粗细
        );

        // 4. 绘制垂直线 (从屏幕顶部到底部，X坐标为鼠标X)
        fg_draw_list->AddLine(
            ImVec2(mouse_pos.x, 0),
            ImVec2(mouse_pos.x, display_size.y),
            crosshair_color,
            1.0f
        );
    }*/
